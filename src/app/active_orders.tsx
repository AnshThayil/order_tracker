...
type ActiveOrderProps = {
    markAsComplete: (newCompletedOrder: OrderProps) => void
}

export default function ActiveOrders({markAsComplete} : ActiveOrderProps) {
    ...
    const handleRemove = (key: string) => {
        setOrderData((current) => current.filter((order) => (order.roomNum + order.guestName + order.timeIn.toLocaleString()) !== key))
    }
            ...
            <GridItem colSpan={1}>
                <Card height={'600px'} overflowY={'auto'} maxHeight={'600px'} margin={'5px'}padding={'15px'} variant={'filled'}>
                    <SimpleGrid margin="5px" columns={orderData.length === 0 ? 1 : 5} spacing={5}>
                        {orderData.length  === 0 ? <Center marginTop={'20%'}>No Active Orders</Center> : orderData.map((object, i) => <Order complete={markAsComplete} remove={handleRemove} key={object.roomNum+object.guestName+object.timeIn.toLocaleString()} timeIn={object.timeIn} timerLen={object.timerLen * 60} guestName={object.guestName} roomNum={object.roomNum} means={object.means} serverName={object.serverName}/>)}
                    </SimpleGrid>
                </Card>
            </GridItem>
